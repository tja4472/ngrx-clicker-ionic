...
export default function (state: BooksState = initialState, action: Action): BooksState {
  switch (action.type) {
    ...
    case BookActions.LOAD_COLLECTION_SUCCESS: {
      ...
      const newBookIds: string[] = newBooks.map(book => book.id);
      const newBookEntities: any = newBooks.reduce(
  ...
  /*
  https://gitter.im/ngrx/store?at=575fcd07814a56807142f311
  
      function list() {
      return Object.assign({}, state, { loading: true });
    }
  
    function listSuccess(entities: NewsItem[]) {
      return {
        loading: false,
        entities
      };
